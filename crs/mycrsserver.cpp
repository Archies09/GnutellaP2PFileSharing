...
#include <arpa/inet.h>//for address
#include <sys/sendfile.h>//for sendfile
#include "crsservertest.h"
#include "myerror.h"
#include <thread>
//SERVER CODE
using namespace std;
...
int serv_call(int argc, char **argv)
{
    ...
    while((acceptfd=accept(socketfd,(struct sockaddr *)&clientaddress,&clientlen))>0)//blocks the process until a client connects to server.Wakes up when connection from client is done)
    {
        ...
        cout<<cliip<<endl;
        write(fileno(listf),cliip,strlen(cliip));
        write(fileno(listf),":",1);
        cout<<"NOTHERE"<<endl;
        if(acceptfd<0)
        ...
        char cliname[100];
        recv(acceptfd,cliname,100,0);
        cout<<cliname<<endl;
        write(fileno(listf),cliname,strlen(cliname));
        write(fileno(listf),"\n",1);
